...
import { Car, Wind, Zap, Armchair, Camera, Sun, Shield, Eye, AlertTriangle,Key,Gauge } from 'lucide-react';
...
const PropertyDetail: NextPage = ({ initialComment, ...props }: any) => {
	...
	//Static Car Features:
	const carFeatures = [
		{ name: 'Heated Seats', iconName: 'Zap', available: true },
		{ name: 'Ventilated Seats', iconName: 'Wind', available: false },
		{ name: 'Power Seats', iconName: 'Armchair', available: true },
		{ name: 'Leather Seats', iconName: 'Armchair', available: false },
		{ name: 'Heated Steering', iconName: 'Car', available: true },
		{ name: 'Smart Key', iconName: 'Key', available: true },
		{ name: 'Cruise Control', iconName: 'Gauge', available: false },
		{ name: 'Navigation', iconName: 'Navigation', available: true },
		{ name: 'Rear Parking Sensor', iconName: 'AlertTriangle', available: true },
		{ name: 'Front Parking Sensor', iconName: 'AlertTriangle', available: false },
		{ name: 'Rear Camera', iconName: 'Camera', available: true },
		{ name: '360° Camera', iconName: 'Camera', available: true },
		{ name: 'Sunroof', iconName: 'Sun', available: false },
		{ name: 'Black Box', iconName: 'Shield', available: true },
		{ name: 'Lane Keep Assist', iconName: 'Car', available: false },
		{ name: 'Blind Spot Warning', iconName: 'Eye', available: true },
	];
	...
	const {
		...
		variables: {
			input: {
				...
				search: {
					brandList: property?.propertyBrand ? [property?.propertyBrand] : [],
				}
			}},
		...
		try {
			...
			await getPropertiesRefetch({
				input: {
					...
					direction: Direction.DESC,
					search: {
						brandList: [property?.propertyBrand],
					}
				}
			});
							...
							<Stack className={'info'}>
								<Stack className={'left-box'}>
									...
									<Stack className={'top-box'}>
										<Typography className={'city'}>{property?.propertyBrand}</Typography>
										<Stack className={'divider'}></Stack>
										<Stack className={'buy-rent-box'}>
											{property?.propertySell && (
												<>
													...
													<Typography className={'buy-rent'}>Sell</Typography>
												</>
											)}
									...
									<Stack className={'bottom-box'}>
										...
										<PaletteIcon sx={{ fontSize: '18px', color: '#476f9b' }} /> <Typography>{property?.propertyColor}</Typography>
										</Stack>
										<Stack className="option">
										<LocalGasStationIcon sx={{ fontSize: '18px', color: '#476f9b' }} /> <Typography>{property?.propertyFuel}</Typography>
										</Stack>
										<Stack className="option">
										<SettingsIcon sx={{ fontSize: '18px', color: '#476f9b' }} /><Typography>{property?.propertyTransmission}</Typography>
										</Stack>
									</Stack>
								</Stack>
							...
							<Stack className={'images'}>
								<Stack className={'main-image'}>
									...
									<Stack className="image-overlay">
										<Stack className="video-button">
											<svg width="20" height="20" viewBox="0 0 24 24" fill="none">
												<path d="M8 5v14l11-7z" fill="white"/>
											</svg>
											<Typography>Video</Typography>
										</Stack>
										<Stack className="view-360-button">
											<svg width="20" height="20" viewBox="0 0 24 24" fill="none">
												<circle cx="12" cy="12" r="10" stroke="white" strokeWidth="2" fill="none"/>
												<path d="M2 12h20M12 2a15.3 15.3 0 0 1 4 10 15.3 15.3 0 0 1-4 10 15.3 15.3 0 0 1-4-10 15.3 15.3 0 0 1 4-10z" stroke="white" strokeWidth="2" fill="none"/>
											</svg>
								...
								<Stack className={'sub-images'}>
									{property?.propertyImages.map((subImg: string, index: number) => {
										...
										return (
											<Stack className={`sub-img-box ${index === 3 && property?.propertyImages.length > 4 ? 'more-images' : ''}`}
												onClick={() => changeImageHandler(subImg)} key={subImg}>
												<img src={imagePath} alt={'sub-image'} />
												{index === 3 && property?.propertyImages.length > 4 && (
													<Stack className="more-overlay">
														<Typography>+{property?.propertyImages.length - 4}</Typography>
													</Stack>
												)}
											</Stack>
										);
						...
						<Stack className={'property-desc-config'}>
							<Stack className={'left-config'}>
								<Stack className={'prop-desc-config'}>
									...
									<Stack className={'bottom'}>
										...
										<Stack className={'info-box'}>
											...
											<Stack className={'right'}>
												<Box component={'div'} className={'info'}>
													<Typography className={'title'}>Odometer:</Typography>
													<Typography className={'data'}>{property?.propertyOdometer} km</Typography>
												</Box>
												<Box component={'div'} className={'info'}>
													<Typography className={'title'}>Options:</Typography>
													<Typography className={'data'}>
														{property?.propertySell && 'Sell'} {property?.propertyRent && 'Rent'}
													</Typography>
												</Box>
											</Stack>
										</Stack>
								...
								<Stack className={'floor-plans-config'}>
									<Typography className={'title'}>Feature</Typography>
									<Stack className={'image-box'}>
										<Stack className={'car-features-grid'}>
											{carFeatures.map((feature, index) => (
											<div
												key={index}
												className={`feature-item ${feature.available ? 'available' : 'unavailable'}`}
											>
									...
									<Stack className={'image-info'}>
										...
										<Stack className={'name-phone-listings'}>
											...
											<Stack className={'phone-number'}>
												<svg xmlns="http://www.w3.org/2000/svg" width="12" height="13" viewBox="0 0 17 16" fill="none">
							...
							<Stack className={'similar-properties-config'}>
								<Stack className={'title-pagination-box'}>
									<Stack className={'title-box'}>
										<Typography className={'main-title'}>You Might Also Like</Typography>
										<Typography className={'sub-title'}>All is releted to your choice</Typography>
									</Stack>
