...
const CommunityDetail: NextPage = ({ initialInput, ...props }: T) => {
	...
	const createCommentHandler = async () => {
		...
		try {
			...
			setComment('');
			await sweetMixinSuccessAlert('Successfully commentd!');
		} catch(error: any) {
	...
	const  updateButtonHandler = async (commentId: string, commentStatus?: CommentStatus.DELETE) =>{
		try {
			...
			if(!updateData?.commentContent && !updateData?.commentStatus)
				throw new Error('Povide data to update your comment');
	...
	if (device === 'mobile') {
		return <div>AVTOMOTIVE BLOG DETAIL PAGE MOBILE</div>;
	} else {
		return (
							...
							<div className="config">
								<Stack className="first-box-config">
										...
										<Stack className="info">
											<Stack className="icon-info">
												{boardArticle?.meLiked && boardArticle?.meLiked[0]?.myFavorite ? (
													<ThumbUpAltIcon style={{color: "#b75348"}}  onClick={() => likeBoArticleHandler(user, boardArticle?._id)}/>
											...
											<Stack className="divider"></Stack>
											<Stack className="icon-info">
												<VisibilityIcon style={{color:'#797570'}} />
												<Typography className="text">{boardArticle?.articleViews}</Typography>
											</Stack>
											<Stack className="divider"></Stack>
											<Stack className="icon-info">
													{total> 0 ? <ChatIcon style={{color:'#83d063'}} /> : <ChatBubbleOutlineRoundedIcon style={{color:'#83d063'}}/>}
									...
									<Stack className="like-and-dislike">
										<Stack className="top">
											<Button>
												{boardArticle?.meLiked && boardArticle?.meLiked[0]?.myFavorite? ( <ThumbUpAltIcon  style={{color: "#b75348"}} onClick={() => likeBoArticleHandler(user, boardArticle?._id)}/>
											):( <ThumbUpOffAltIcon style={{color: "#b75348"}} onClick={() => likeBoArticleHandler(user, boardArticle?._id)}/>
											)}
												<Typography className="text">{boardArticle?.articleLikes}</Typography>
											</Button>
										</Stack>
